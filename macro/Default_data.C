...
void TMVAClassificationApplication__DEF_CH__data( TString myMethodList = "" )
{
   ...
   TH1F *_DEF_CH__data_obs(0);
   _DEF_CH__data_obs = new TH1F("_DEF_CH__data_obs", "_DEF_CH__data_obs", nbin, -1.0, 1.0);
   ...
   for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
      ...
      // Fill histograms
      _DEF_CH__data_obs -> Fill( MVAscore );
   }
   ...
   TFile *target  = new TFile( "_DEF_CH__data.root","RECREATE" );
   _DEF_CH__data_obs -> Write();
   target->Close();

   std::cout << "--- Created root file: \"_DEF_CH__data.root\" containing the MVA output histograms" << std::endl;
...
int main( int argc, char** argv )
{
   ...
   TMVAClassificationApplication__DEF_CH__data(methodList);
   return 0;
}
