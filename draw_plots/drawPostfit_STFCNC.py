...
#mclist = ["dy", "stv", "zz", "wz", "tth", "ttv", "ttjet"] ## st, ww are vanished because they are zero entry
mclist = ["dy", "stv", "zz", "wz", "ttv", "ttjet", "others"]
colorlist = ["kOrange+1", "kMagenta+2", "kCyan-1", "kBlue-6", "kRed+4", "kRed", "kMagenta+2"]#colorlist = ["kOrange+1", "kMagenta+1", "kMagenta+2", "kCyan-1", "kAzure+6", "kBlue-6", "kRed+3", "kRed+4", "kRed"]
#colorlist = ["kOrange+1", "kMagenta+2", "kCyan-1", "kAzure+6", "kRed+3", "kRed+4", "kRed"]

f = TFile.Open("fitDiagnostics_tZc.root")
...
#def drawplot (hdata, hbkg, hsig, channel):
    ...
    c.SaveAs("Postfit_%s_stzct.png"%channel)
...
for i, channel in enumerate(chlist):
    ...
    for j, mc in enumerate(mclist):
        print channel, mc
        htemp = f.Get("shapes_fit_s/%s/%s"%(channel, mc))
        hs0.Add(htemp)
        htemp.SetLineColor(kBlack)
        htemp.SetFillColor(eval(colorlist[j]))
        hs.Add(htemp)
        leg.AddEntry(htemp, mc, "F")
    datagraph = f.Get("shapes_fit_s/%s/data"%channel)
    ...
    for i in range(0,nPoints-1):
        ...
        hdata.Fill(px,py)
    hsig = f.Get("shapes_fit_s/%s/stzct"%channel)
