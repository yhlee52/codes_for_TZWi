...
name = "bkgEst"
result = "fitresult_%s_STFCNC.txt" % name
baseDir = "./hist"
...
for i, ch in enumerate(chlist):
    ...
    hTTrd = f.Get("STCR/hZ_mass/Data") # For fitting (allZ)
    hTTtt = f.Get("STCR/hZ_mass/ttJets")
    hTTDY = f.Get("STCR/hZ_mass/DYJets")
    hTTWZ = f.Get("STCR/hZ_mass/WZ")
    hTTTV = f.Get("STCR/hZ_mass/ttV")
    hTTSR = f.Get("STSR/hZ_mass/ttJets")
...
### To extract expected entries
    hexpWZ = f.Get("STSR/hW_MT/WZ")
    hexpZZ = f.Get("STSR/hW_MT/ZZ")
    hexpDY = f.Get("STSR/hW_MT/DYJets")
    ...
    for i, mc in enumerate(mclist):
        hista = f.Get("WZCR/hnGoodJet/%s" % mc)    
        histb = f.Get("WZCR_1jet/hnBjet/%s" % mc)
        n1j0b = hista.Integral(2,2)
        n1j0berr = calcError(hista, 2, 3)
        n123j0b = hista.Integral()
        n123j0berr = calcError(hista, 1, 5)
        ratio0b.append(n1j0b/n123j0b)
        ratio0berr.append(getRErr(n1j0b, n123j0b, n1j0berr, n123j0berr, 1))
        n1b1j = histb.Integral(2,2)
        n1b1jerr = calcError(histb, 2, 3)
        n0b1j = histb.Integral(1,1)
        n0b1jerr = calcError(histb, 1, 2)
        ratio1j.append(n1b1j/n0b1j)
        ratio1jerr.append(getRErr(n1b1j, n0b1j, n1b1jerr, n0b1jerr, 1))
        ### Should be changed!!! ###
        estedWZCR = 35900 * totMC * fraclist[i] * ratio0b[i] * ratio1j[i]
        errWZCR = estedWZCR * getRErr2(fraclist[i], ratio0b[i], ratio1j[i], errlist[i], ratio0berr[i], ratio1jerr[i])
        ###
        file = open(result, "a")
        wline = "%s %s %f %f %f %f %f %f %f %f %f %f %f\n" %(ch, mc, hnlist[i]*35900, 35900*totMC*fraclist[i], 35900*totMC*errlist[i], ratio0b[i], ratio0berr[i], ratio1j[i], ratio1jerr[i], estedWZCR, errWZCR, nexplist[i]*35900, experrlist[i]*35900)
